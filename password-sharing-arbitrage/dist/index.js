...
function detectGeoSplit(sessions, config) {
    const accountSessions = sessions.reduce((acc, session) => {
        if (!acc[session.accountId])
...
function detectDeviceBloom(accounts, config) {
    const accountMetrics = {};
    let count = 0;
...
function detectConcurrencyBreach(accounts, sessions, config) {
    const accountMetrics = {};
    let count = 0;
    accounts.forEach(acc => {
        ...
        accSessions.forEach(s => {
            const overlapping = accSessions.filter(other => other.startTime < s.startTime + s.duration &&
                other.startTime + other.duration > s.startTime).length;
...
function detectSessionRelay(sessions, config) {
    const accountSessions = sessions.reduce((acc, session) => {
        if (!acc[session.accountId])
            acc[session.accountId] = [];
        acc[session.accountId].push(session);
    ...
    for (const [accId, sess] of Object.entries(accountSessions)) {
        ...
        accountMetrics[accId] = { rate, sessions: sess.length };
        if (rate > config.sessionRelayThreshold)
            totalCount++;
    }
    return { count: totalCount, accounts: accountMetrics };
}
function confidenceScore(flags) {
    const weights = {
        geo_split: 0.3,
        device_bloom: 0.2,
    ...
    const score = Object.entries(weights).reduce((acc, [k, w]) => acc + (flags[k] ? w : 0), 0);
    return Math.min(score, 0.95);
}
function determineEnforcement(riskScore, confidence, config) {
...
function runArbitrageAnalysis(data, config) {
    ...
    return {
        ...
        primary_vectors: Object.keys(arbitrage).filter(key => arbitrage[key] > 0),
        baselines: {
...
function main() {
    ...
    const raw = JSON.parse(readFileSync(inputFilePath, 'utf-8'));
